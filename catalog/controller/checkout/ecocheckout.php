 ...
 */
class ControllerCheckoutEcocheckout extends Controller {
    ...
    public function index() {
        ...
        $this->load->language('checkout/ecocheckout');
        $this->load->model('checkout/ecocheckout');
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('checkout/ecocheckout')
        ];
        ...
        // URLs for AJAX
        $data['url_search_cities'] = $this->url->link('checkout/ecocheckout/searchCities', '', true);
        $data['url_get_departments'] = $this->url->link('checkout/ecocheckout/getDepartments', '', true);
        $data['url_update_shipping'] = $this->url->link('checkout/ecocheckout/updateShipping', '', true);
        $data['url_update_cart'] = $this->url->link('checkout/ecocheckout/updateCart', '', true);
        $data['url_remove_product'] = $this->url->link('checkout/ecocheckout/removeProduct', '', true);
        $data['url_create_order'] = $this->url->link('checkout/ecocheckout/createOrder', '', true);
        ...
        $this->response->setOutput($this->load->view('checkout/ecocheckout', $data));
    }
    ...
    private function getCustomerData() {
        ...
        if (isset($this->session->data['ecocheckout'])) {
            $customer = array_merge($customer, $this->session->data['ecocheckout']);
        }
    ...
    public function updateShipping() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            // Save to session
            if (!isset($this->session->data['ecocheckout'])) {
                $this->session->data['ecocheckout'] = [];
            }
            $this->session->data['ecocheckout']['city_ref'] = $city_ref;
            $this->session->data['ecocheckout']['department_ref'] = $department_ref;
            $this->session->data['ecocheckout']['shipping_code'] = $shipping_code;
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($json['errors'])) {
                ...
                if ($payment_code == 'liqpay') {
                    $json['redirect'] = $this->url->link('checkout/ecocheckout/liqpay', '', true);
                } else {
                    // Clear cart and redirect to success
                    $this->cart->clear();
                    unset($this->session->data['shipping_method']);
                    unset($this->session->data['payment_method']);
                    unset($this->session->data['ecocheckout']);
    ...
    public function liqpay() {
        ...
        $params = [
            ...
            'result_url' => $this->url->link('checkout/ecocheckout/liqpayReturn', '', true),
            'server_url' => $this->url->link('checkout/ecocheckout/liqpayCallback', '', true),
            'language' => 'uk'
        ];
        ...
        $this->response->setOutput($this->load->view('checkout/ecocheckout_liqpay', $data));
    }
    ...
    public function liqpayReturn() {
        ...
        $this->response->redirect($this->url->link('checkout/ecocheckout', '', true));
    }
}
