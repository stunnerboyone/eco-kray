...
// и выполняем её.
$file_content = file('../index.php');  // Читаємо файл
$filtered = preg_grep('/VERSION/', $file_content);  // Фільтруємо його
reset($filtered);  // Тепер передаємо змінну в reset()
...
if (function_exists('getallheaders')) {
    ...
    $log->write("All headers: " . print_r($headers, true));
} else {
    $log->write("getallheaders() is not available.");
}

// Логування конкретного заголовка Authorization
if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    $log->write('Authorization Header: ' . $_SERVER['HTTP_AUTHORIZATION']);
} else {
    $log->write('Authorization Header not set');
}
